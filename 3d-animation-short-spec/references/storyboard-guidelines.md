...
6. **Double-binding** — `[char:角色名-01] [char:角色名-02] ... [scene:场景名] [hook: visual-joke]` — exact character card names, scene card name, hook type. These are storyboard-only reference markers; the video model strips them at prompt-writing time.
...
If the user picked the visualization mode in the storyboard-mode choice card, ALSO produce one multi-panel pencil storyboard image per table row on top of the text storyboards document. The text storyboards document remains the authoritative prompt-writing reference; the pencil images are human-review-only.
...
- **Double-binding labels (top-right corner, mandatory on image)**:
  ...
  - These labels are storyboard-only reference markers; they are stripped at prompt-writing time.
...
- Keep the draft as a video-prompt reference asset only, not final art.
...
- Approve storyboards and write shot video prompts (recommended)
